def updateCcaSlotInScdFile(self, fileName = '', slot = '',  indexOf3AesPesk = '', newSn = False):    
    '''    Will update values of slot, sn, ua in specified scd file. Only one card  exist in the file.    
    Since we can't upload duplicated sn in Db, so the method will also update sn and ua when change the value of Slot.    
    '''    
    if "\\" not in fileName:        
